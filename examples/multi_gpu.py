# 参考文献: https://blog.csdn.net/daydayup_668819/article/details/80006229
# encoding:utf-8
import math
import tempfile
import time
...
IMAGE_PIXELS = 28
# 定义默认训练参数和数据路径
flags.DEFINE_string('data_dir', '/tmp/data/mnist/', 'Directory  for storing mnist data')
...
def main(unused_argv):
    ...
    if FLAGS.job_name is None or FLAGS.job_name == '':
        ...
        print
        'job_name : %s' % FLAGS.job_name
    if FLAGS.task_index is None or FLAGS.task_index == '':
        raise ValueError('Must specify an explicit task_index!')
    else:
        print
        'task_index : %d' % FLAGS.task_index
    ...
    with tf.device(tf.train.replica_device_setter(
        ...
        if is_chief:
            ...
            print
            'Worker %d: Waiting for session to be initaialized...' % FLAGS.task_index
        ...
        while True:
            ...
            now = time.time()
            print
            '%f: Worker %d: traing step %d dome (global step:%d)' % (now, FLAGS.task_index, local_step, step)
        ...
        time_end = time.time()
        print
        'Training ends @ %f' % time_end
        train_time = time_end - time_begin
        print
        'Training elapsed time:%f s' % train_time

        val_feed = {x: mnist.validation.images, y_: mnist.validation.labels}
        val_xent = sess.run(cross_entropy, feed_dict=val_feed)
        print
        'After %d training step(s), validation cross entropy = %g' % (FLAGS.train_steps, val_xent)
    sess.close()
